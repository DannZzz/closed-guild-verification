...
export default class Models<T extends keyof typeof models, M extends Get<T>> {
    ...
    async findOrCreate(_id: string, additional?: Omit<M, "_id">): Promise<M> {
        ...
        if (!data) {
            return await this.create({...(additional || {}), _id} as M);
        } else {
            return data as any;
        }
    }
 ...
 */
export async function changeMoney(_id: string, amount: number, type: keyof Currency, notRound: boolean = false): Promise<void> {
